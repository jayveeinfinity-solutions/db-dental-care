    ...
    <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
        <nav>
            <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
                ...
                @foreach ($segments as $index => $segment)
                    @continue($segment === 'admin')
                    <li class="text-sm pl-2 capitalize leading-normal text-white 
                            before:float-left before:pr-2 before:text-white before:content-['/']"
                        aria-current="{{ $loop->last ? 'page' : '' }}">
                        @if (!$loop->last)
                            <a href="{{ url(implode('/', array_slice($segments, 0, $index + 1))) }}" class="opacity-70 hover:opacity-100">
                                {{ str_replace('-', ' ', $segment) }}
                            </a>
                        @else
                            {{ str_replace('-', ' ', $segment) }}
            ...
            <h6 class="mb-0 font-bold text-white capitalize">
                {{ ucfirst(last($segments) ?? 'Dashboard') }}
            </h6>
        </nav>
        ...
        <div class="flex items-center mt-2 grow sm:mt-0 sm:mr-6 md:mr-0 lg:flex lg:basis-auto">
            ...
            <ul class="flex flex-row justify-end pl-0 mb-0 list-none md-max:w-full">
                ...
                <li class="flex items-center">
                    <a href="javascript:void(0)" class="flex text-sm bg-gray-800 rounded-full md:me-0 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600">
                ...
                <li class="flex items-center pl-4 xl:hidden-important">
